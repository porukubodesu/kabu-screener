...
# 事業説明の文抽出のノイズは3層:
# 法務・開示定型文 = 無条件で落とす(事業の説明が同居することはまず無い)
_BIZ_NOISE_HARD = (
    "インサイダー取引", "内閣府令", "特定上場会社", "軽微基準",
    "セグメント別の記載", "セグメント情報の記載", "記載を省略",
)
# 背景系 = 市場の語り・開示の但し書きに多いが、
# 「単一セグメントでありますが、◯◯事業を行っております」のように
# 本業の説明と同居しうる。強い動詞+現在形で終わる文だけ残す
_BIZ_NOISE_AMBI = (
    "従来の", "近年", "とりまく環境", "取り巻く環境", "市場環境", "市場規模",
    "単一セグメント",
)
# 理念系 = 「〜を目指し、◯◯サービスを提供」と同居することが多い。
# 強い動詞があれば残す
_BIZ_NOISE_SOFT = (
...
_BIZ_CONCRETE = (
    ...
    "を提供", "を運営", "を販売", "を製造", "を開発", "を展開",
    "提供し", "運営し", "販売し", "製造し", "開発し", "展開し",
    # 汎用の活動動詞(「製造・販売を行っております」のような中黒連結を拾う)
    "を行って", "を営んで", "を手掛けて", "を手がけて",
    "サービス", "主な事業", "主として", "を中心に", "ブランド",
)
...
def extract_business(text: str, limit: int = 340) -> str:
    ...
    for s in sentences:
        ...
        if any(n in s for n in _BIZ_NOISE_AMBI) and not (
                strong and s.rstrip("。").endswith(("おります", "います", "ます"))):
            continue
        # 理念系: 強い動詞と同居していれば残す
        if any(n in s for n in _BIZ_NOISE_SOFT) and not strong:
            continue
        picked.append(s)
        total += len(s)
        if total >= limit:
