...
| 終値(時価総額の概算用) | **J-Quants API**(JPX公式、無料プラン) | 12週遅延。https://jpx-jquants.com/ で無料登録 → `.env` に `JQUANTS_MAIL` / `JQUANTS_PASSWORD`。未設定なら時価総額欄は「-」 |
...
- **時価総額は概算・12週遅延**: J-Quants無料プランの終値×推定株式数(純資産÷BPS)。正確な発行済株式数ベースではない。株価の遅延なし化は有料プラン or 別ソース検討
